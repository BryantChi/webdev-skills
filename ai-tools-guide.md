---
name: AI 工具使用指南
description: 2026 最新版：如何在各種 AI 編程助手與 CLI 工具中加載和使用此 Skills 系統
---
...
| ------- | ---- | ------ | ------------ | -------- |
| **Antigravity** | Agent IDE | ✅ Native | 自動讀取 Skills | ⭐⭐⭐⭐⭐ |
...
| **OpenCode CLI** | CLI | ✅ 相容 | `--plan` | ⭐⭐⭐⭐⭐ |
| **Aider** | CLI Chat | ✅ 相容 | `/read` 或 `/add` | ⭐⭐⭐⭐ |
| **Claude CLI** | CLI | ✅ 相容 | Project Mode | ⭐⭐⭐⭐ |
| **Copilot** | VS Code Ext | ✅ 相容 | `#file:` 引用 | ⭐⭐⭐⭐ |
| **Open Interpreter** | CLI | ⚠️ 部分 | System Message | ⭐⭐⭐ |
...
### Goose (Google's Agent)
> 2026 強力推薦的 CLI Agent，擅長執行複雜任務。

**啟動方式：**
```bash
# 載入 SOP 並啟動 Session
goose run --instruction-file .agent/skills/webdev/SKILL.md
```

**在 Session 中：**
```text
Goose> 幫我建立一個 Next.js 電商網站
```
Goose 會自動讀取 `SKILL.md` 中的 SOP，並開始執行 Phase 1 推理。
...
```bash
# 啟動並載入核心 Skill
aider --read .agent/skills/webdev/SKILL.md --read .agent/skills/webdev/ui/no-ai-feel.md
```

**技巧：** Aider 擅長編輯，建議先用 `SKILL.md` 生成計畫，再讓 Aider 執行 `architect` 模式。
```text
> /architect 根據 SKILL.md 的 Phase 1 進行推理，並規劃專案結構
```

### Claude CLI (Anthropic)
> 適合專案級別的分析與生成。

**設定 `claude.json` (若支援) 或直接參數啟動：**
```bash
claude --system "$(cat .agent/skills/webdev/SKILL.md)" "幫我建立一個網站"
```
...
**啟動：**
```bash
interpreter --system_message_file .agent/skills/webdev/SKILL.md
```
*注意：Open Interpreter 權限較大，建議在 Container 或虛擬環境中使用。*
...
```bash
# 解釋規範
gh copilot explain "$(cat .agent/skills/webdev/ui/no-ai-feel.md)"

# 生成指令
gh copilot suggest "建立 Next.js 專案並安裝 Tailwind"
```

### OpenCode.ai CLI
> 2026 新興的高效能 Agent，以極快的 Context 索引與「計畫模式 (Plan Mode)」著稱。

**啟動方式：**

```bash
# 啟動並載入 SOP
opencode start --plan .agent/skills/webdev/SKILL.md
```
